...
- NEXT (Node/TS = testable, self-merge): connect strict-null tsc fixes; chat port-8106 collision (compose). Python-service fixes (registry Stripe honesty+state-secret, admin JWT-aud + unsubscribe fail-closed) → STAGE branches for Mac morning test, do NOT self-merge untested (sandbox has py3.10, repos need 3.11/3.12).
