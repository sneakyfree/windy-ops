...
- NEXT tonight (Node = testable): windy-chat AgentRunner._request timeout (core grandma-loop bug). Then connect strict-null tsc fixes. Python-service fixes (registry Stripe honesty, admin JWT-aud/unsubscribe) → STAGE for Mac morning test, do NOT self-merge untested.
